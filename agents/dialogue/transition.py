...
import autogen
from agents.utils.registry import transition_registry
from agents.ontology.config.dialogue import PodcastConfig
...
def podcast_state_transition(
    ...
    logger.info(f"Number of agents: {len(groupchat.agents)}. Agents: {groupchat.agent_names}")
    character_agents = [
        a for a, name in zip(groupchat.agents, groupchat.agent_names) if name in character_cfg.guest_names]
    host_agents = [
        a for a, name in zip(groupchat.agents, groupchat.agent_names) if name in character_cfg.host_names]
    if len(messages) >= max_round-1:
        return groupchat.agents_by_name("script_parser")  # Assuming the script parser is the last agent
    if last_speaker.name in character_cfg.guest_names + character_cfg.host_names:
        next_speaker = weighted_choice(
            character_agents + host_agents, last_speaker,
            host_chance_factor=0.5, 
            host=groupchat.agent_by_name("Podcast Host")
        )
        return next_speaker
